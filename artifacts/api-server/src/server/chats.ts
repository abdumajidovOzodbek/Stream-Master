...
router.get("/contacts/search", async (req: Request, res: Response) => {
  ...
  try {
    res.json(await searchContacts(client, q, limit));
  } catch (err) {
...
router.get("/messages", async (req: Request, res: Response) => {
  ...
  if (!chatId) { res.status(400).json({ error: "Missing required query param: chatId" }); return; }
  const limit = Math.min(Number(req.query["limit"] ?? 50) || 50, 100);
  const offsetIdRaw = req.query["offsetId"];
  const offsetId = offsetIdRaw ? Number(offsetIdRaw) : undefined;
  try {
    res.json(await listMessages(client, chatId, limit, offsetId));
  } catch (err) {
...
router.get("/search", async (req: Request, res: Response) => {
  ...
  const chatId = (req.query["chatId"] as string | undefined)?.trim();
  const q = (req.query["q"] as string | undefined)?.trim();
  if (!chatId || !q) { res.status(400).json({ error: "Missing required query params: chatId, q" }); return; }
  const limit = Math.min(Number(req.query["limit"] ?? 20) || 20, 50);
  try {
    res.json(await searchMessages(client, chatId, q, limit));
  } catch (err) {
...
router.post("/messages", async (req: Request, res: Response) => {
  const client = await getAuthedClient(req, res);
  if (!client) return;
  const body = (req.body ?? {}) as { chatId?: string; text?: string; replyToMsgId?: number; scheduleDate?: number };
  const chatId = body.chatId?.trim();
  const text = body.text;
  if (!chatId || !text || !text.trim()) { res.status(400).json({ error: "chatId and text are required" }); return; }
  try {
    res.json(await sendChatMessage(client, chatId, text, body.replyToMsgId, body.scheduleDate));
  } catch (err) {
...
router.get("/og", async (req: Request, res: Response) => {
  ...
  if (!url || !/^https?:\/\//i.test(url)) { res.status(400).json({ error: "Missing or invalid url" }); return; }
  try {
...
router.get("/folders", async (req: Request, res: Response) => {
  ...
  if (!client) return;
  try {
    res.json(await getDialogFolders(client));
  } catch (err) {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const chatId = body.chatId?.trim();
    if (!file || !chatId) { res.status(400).json({ error: "file and chatId are required" }); return; }
    const caption = body.caption?.trim();
    const replyToMsgId = Number(body.replyToMsgId) || undefined;
    try {
      res.json(await sendMediaFile(client, chatId, file.buffer, file.originalname, caption, replyToMsgId));
    } catch (err) {
...
router.post("/dialogs/:chatId/read", async (req: Request, res: Response) => {
  ...
  const maxId = typeof body.maxId === "number" && Number.isFinite(body.maxId) ? body.maxId : undefined;
  try {
    res.json(await markChatRead(client, chatId, maxId));
  } catch (err) {
...
router.get("/users/:peerId", async (req: Request, res: Response) => {
  ...
  try {
    res.json(await getUserInfo(client, peerId));
  } catch (err) {
    handleError(req, res, err, "Failed to fetch user info");
  }
});
...
router.get("/media/:chatId/:msgId", async (req: Request, res: Response) => {
  ...
  if (!chatId || !Number.isFinite(msgId)) { res.status(400).json({ error: "Invalid chatId or msgId" }); return; }
  try {
    const opened = await openMessageMedia(client, chatId, msgId, thumb);
    if (!opened) { res.status(404).json({ error: "Media not found" }); return; }
    ...
    if (opened.fullBuffer) {
      ...
      if (forceDownload) {
        res.setHeader("Content-Disposition", `attachment; filename="${encodeURIComponent(opened.info.fileName)}"`);
      }
